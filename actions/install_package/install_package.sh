...
docker build -t base-${package_name}:latest -f ${path_to_dockerfile} . --build-arg GCC_VERSION=${gcc_version} --build-arg CLANG_VERSION=${clang_version} --build-arg CMAKE_VERSION=${cmake_version} --build-arg UBUNTU_VERSION=${ubuntu_version} --build-arg MADNESS_VERSION=${madness_version} --build-arg CATCH2_VERSION=${catch2_version} --build-arg SPDLOG_VERSION=${spdlog_version} --build-arg CEREAL_VERSION=${cereal_version} --build-arg LIBFORT_VERSION=${libfort_version} --build-arg LIBBOOST=${libboost_version} --build-arg PARENT_IMAGE_NAME=${parent_image_name}
